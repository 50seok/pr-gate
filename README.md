...
리뷰 1회가 얼마나 썼는지 PR 코멘트 맨 아래와 Actions 로그에 남는다.

```
토큰 입력 9 · 출력 997 · 캐시생성 57,616 · 종량환산 $0.1596
```

`--output-format json`으로 CLI가 내는 usage를 그대로 읽는다. **종량환산은 실제 청구가
아니다** — 구독 토큰으로 도니 돈은 안 나가고, 대신 주간 사용량 한도를 그만큼 쓴다는
뜻으로 읽으면 된다.

**캐시생성이 대부분을 차지한다.** 매 실행이 새 러너·새 세션이라 `cache_read`가 항상 0이고
시스템 프롬프트·도구 정의를 매번 새로 올린다. 즉 diff 크기보다 이 고정비가 더 클 수 있다
— PR을 잘게 쪼갤수록 총량이 늘어나는 구조라는 뜻이다.

**벤더 중립은 유지된다.** 래퍼 JSON이 없는 CLI(codex·grok)면 집계 줄만 빠지고 리뷰는
그대로 돈다. 누적 집계는 따로 안 만들었다 — 코멘트와 로그에 남아 있으니 필요해지면
`gh run view --log`로 긁으면 된다.
